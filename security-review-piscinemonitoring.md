...
- PRs opened: 1
  - #45 — https://github.com/FlorianCasse/PiscineMonitoring/pull/45 (service worker cross-origin bypass)
- Issues opened: 5
  - #46 — https://github.com/FlorianCasse/PiscineMonitoring/issues/46 (SRI on CDN scripts, HIGH)
  - #47 — https://github.com/FlorianCasse/PiscineMonitoring/issues/47 (CSP `unsafe-eval` / `unsafe-inline`, MEDIUM)
  - #48 — https://github.com/FlorianCasse/PiscineMonitoring/issues/48 (validate `client_payload` before writing JSON, MEDIUM)
  - #49 — https://github.com/FlorianCasse/PiscineMonitoring/issues/49 (home coordinates + name exposed, LOW)
  - #50 — https://github.com/FlorianCasse/PiscineMonitoring/issues/50 (manifest `id` hygiene, LOW)
...
- **Description:** Three production scripts (React 18.3.1, ReactDOM 18.3.1, Babel Standalone 7.29.0) are loaded from `https://unpkg.com/...` without `integrity=` attributes. `unpkg` resolves a semver-pinned URL but ultimately serves arbitrary content from the registry/CDN; a registry takeover, CDN compromise, or upstream tag replacement would execute attacker-controlled JavaScript with full DOM and `localStorage` access. The existing CSP further permits `'unsafe-inline'` and `'unsafe-eval'` (required by Babel), so any injected script has minimal sandbox.
- **Remediation:** Add `integrity="sha384-..."` attributes (with `crossorigin="anonymous"` already present) on all three `<script src="https://unpkg.com/...">` tags. Hashes should be computed against the exact pinned bytes (`curl -sL <url> | openssl dgst -sha384 -binary | openssl base64 -A`). Alternatively, vendor the three libraries into the repo, which removes the SRI requirement, lets the CSP drop `https://unpkg.com` from `script-src`, and eliminates third-party CDN trust entirely. Self-hosting is recommended given the site is a single-user dashboard.
- **PR-ready:** no (SRI hash computation requires verified network access to the CDN; placeholder hashes would brick production)
- **Action taken:** Issue #46 https://github.com/FlorianCasse/PiscineMonitoring/issues/46
...
- **Action taken:** Issue #47 https://github.com/FlorianCasse/PiscineMonitoring/issues/47
...
### [MEDIUM] GitHub Actions writes `status.json` directly from unvalidated payload
...
- **PR-ready:** no (acceptable ranges need owner sign-off)
- **Action taken:** Issue #48 https://github.com/FlorianCasse/PiscineMonitoring/issues/48
...
- **Action taken:** Issue #49 https://github.com/FlorianCasse/PiscineMonitoring/issues/49

### [LOW] PWA `manifest.json` lacks `id` field
...
- **Remediation:** Add `"id": "/"` (or a stable opaque identifier).
- **PR-ready:** no (cosmetic)
- **Action taken:** Issue #50 https://github.com/FlorianCasse/PiscineMonitoring/issues/50
...
## Label note
The MCP tool used to apply labels auto-created severity labels in uppercase (`MEDIUM`) rather than the requested lowercase. The repository's existing `high` label remains lowercase. Owner can rename via `Settings → Labels` if uniform casing is desired. All issues and the PR are labelled `security`, `Claude`, and a severity tag.
